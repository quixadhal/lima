...
mixed glob1(mixed a, mixed b);
mixed has_magic(mixed x);
mixed fnmatch(mixed a, mixed b);
varargs mixed translate( string, int);
...
int 
fnmatch( string name, string pattern )
{
    string 	result;
...
// 1 means valid.
varargs  
string translate( string pat, int flag )
{
    int 	i, j, n;
    string	res, stuff;
