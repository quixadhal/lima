...
varargs void receive_private_msg();
int query_hp();
int query_max_hp();
...
// just call set_weapon_class() and set_to_hit_bonus()
int to_hit_base() {
    return 50;
}
...
class combat_result array take_a_swing(object target)
{
	...
	int bn = weapon->query_to_hit_bonus(target);
	int hs = hit_skill();
...
#else
    chance = target->to_hit_base() + weapon->query_to_hit_bonus(target)
	...
	res->kind = query_weapon_damage_kind();
    }
